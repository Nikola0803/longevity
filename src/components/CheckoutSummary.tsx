...
const DEFAULT_FREE_SHIPPING_THRESHOLD = 350;
...
export default function CheckoutSummary({
  ...
  const total = subtotal + shippingDollars;

  return (
    <>
      {items.length > 0 && showFreeShippingProgress && (
      ...
      <div className="p-4 border-t border-background-200/60 space-y-2">
        ...
        <div className="flex items-center justify-between text-[13px]">
          ...
          <span className="font-display text-foreground-200">
            {shippingCents === 0 ? "Free" : `$${shippingDollars.toFixed(2)}`}
          </span>
        </div>
        <div className="flex items-center justify-between text-[16px] font-semibold pt-2 border-t border-background-200/40">
          ...
          <span className="font-display text-foreground-100">${total.toFixed(2)}</span>
        </div>
      </div>
    </>
  );
