...
export default function BestSellerCard({
  ...
  // All hooks run unconditionally on every render (even before the product
  // list has finished loading) — the `if (!base) return null` guard below
  // happens after every hook call, not before, so this stays rules-of-hooks
  // safe once `base` starts out undefined and then resolves post-fetch.
  const products = useProducts();
  const base = getProduct(products, slug);
  const variants = base ? getVariants(products, base.name) : [];
