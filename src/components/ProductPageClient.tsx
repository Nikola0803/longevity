...
export default function ProductPageClient({ slug }: { slug: string }) {
  // All hooks called unconditionally first — the loading/not-found guard
  // below happens after every hook, so this stays rules-of-hooks safe
  // while `products` is empty during the initial fetch.
    ...
    return (
      <div className="min-h-screen bg-background-800 text-foreground-100 flex items-center justify-center">
        {loading ? (
          ...
          <p className="text-foreground-400">Product not found.</p>
        )}
      </div>
    );
  }
        ...
        <section className="w-full max-w-[1440px] mx-auto px-6 md:px-10 py-10 md:py-14">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14">
            ...
            <div className="lg:col-span-6 xl:col-span-7">
              ...
              {/* Spray kit contents — sprays ship as a full kit, not just
                  the vial, so make that explicit before checkout. */}
              {product.category === "Sprays" && (
                <div className="mb-8 rounded-lg border border-primary-500/25 bg-primary-500/[0.04] p-5">
                  <div className="flex items-center gap-2 mb-3">
                    <i className="ri-gift-2-line text-[16px] text-primary-500"></i>
                    <h3 className="font-display text-[15px] text-foreground-100">What's in the box</h3>
                  </div>
                  <ul className="space-y-2 text-[13px] text-foreground-300">
                    <li className="flex items-start gap-2">
                      <i className="ri-checkbox-circle-line text-primary-500 mt-0.5 shrink-0"></i>
                      1x vial of {product.name}
                    </li>
                    <li className="flex items-start gap-2">
                      <i className="ri-checkbox-circle-line text-primary-500 mt-0.5 shrink-0"></i>
                      1x vial of sterile (bacteriostatic) water
                    </li>
                    <li className="flex items-start gap-2">
                      <i className="ri-checkbox-circle-line text-primary-500 mt-0.5 shrink-0"></i>
                      1x empty sprayer vial
                    </li>
                    <li className="flex items-start gap-2">
                      <i className="ri-checkbox-circle-line text-primary-500 mt-0.5 shrink-0"></i>
                      <span>
                    ...
                    <table className="w-full text-left">
                      <tbody>
                        {[
                          ...
                          ["Format", product.category === "Sprays" ? "Kit — peptide vial + sterile water + sprayer" : "Lyophilized powder"],
