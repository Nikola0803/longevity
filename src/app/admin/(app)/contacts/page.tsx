          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map(({ contact, ltv, lastOrder, orderCount }) => (
                <tr key={contact.id} className="border-b border-cc-background-100 last:border-0">
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-full bg-cc-secondary-100 text-cc-secondary-900 flex items-center justify-center text-xs font-semibold shrink-0">
                        {initials(contact.email)}
                      </div>
                      <span className="text-cc-foreground-800">{contact.email}</span>
                    </div>
                  </td>
                  <td className="py-3 px-4 text-cc-foreground-600 text-xs">
                  ...
                  <td className="py-3 px-4 text-cc-foreground-600 text-xs">{shortDate(contact.createdAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
